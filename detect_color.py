...
class ColorDetector():
    ...
    def detect_color(self):
        ...
        upper = None
        if self.color == "red":
            lower = np.array([161, 153, 153], dtype = "uint8") # lower_red
            upper = np.array([179, 255, 255], dtype = "uint8") # upper_red
            ...
            lower = np.array([13, 93, 220], dtype = "uint8")
            upper = np.array([21, 178, 255], dtype = "uint8")
        elif self.color == "green":
            lower = np.array([50, 58, 63], dtype = "uint8")  # lower_green
            upper = np.array([69, 153, 153], dtype = "uint8") # upper_green
        ...
        cv2.imwrite("images/%s.jpg" % self.color, detected_output)
        cv2.waitKey(0)
